...
Depo ayarlarından **Settings → Pages → Source: Deploy from a branch** seçilip
`main` dalı ve `/ (root)` klasörü gösterilmelidir.

Sitede kök dizinden başlayan (`/css/...` gibi) mutlak yol kullanılmamıştır;
tüm bağlantılar görecelidir. Bu nedenle site hem `kullanici.github.io/depo/`
altında hem de ileride kendi alan adına taşındığında değişiklik gerektirmeden
çalışır.

Depoda iki yardımcı dosya bulunur:

- `.nojekyll` — GitHub'ın Jekyll işlemcisini devre dışı bırakır, yayın süresini
  kısaltır ve dosya adlarının olduğu gibi korunmasını sağlar.
- `404.html` — hatalı bağlantılarda kurumsal görünümlü bir hata sayfası gösterir.

> GitHub Pages dosya adlarında **büyük/küçük harf ayrımı yapar.** Kendi
> bilgisayarınızda çalışan bir bağlantı, harf farkı yüzünden yayında 404
> verebilir. Yeni dosya eklerken adları küçük harf ve Türkçe karakter içermeyecek
> biçimde tutunuz (`ilceler/sivrihisar.html` gibi).

## Sürüm notları

**Mobil düzeltmeleri**

- Menüdeki bir bağlantının hedefi boş olduğu için (`href="#"`) tarayıcı geçersiz
  bir seçici sorguluyor ve JavaScript dosyası ilk satırlarında tamamen
  duruyordu. Bunun sonucunda Sıkça Sorulan Sorular açılmıyor, mobil menü
  çalışmıyor ve açılır ilçe paneli ekranın üzerine görünmez biçimde binerek
  dokunuşları yanlış bağlantılara yönlendiriyordu. Hatalı çağrı korumaya
  alınmış, dosyadaki tüm seçici sorguları güvenli hale getirilmiştir.
- Sıkça Sorulan Sorular panelleri `grid-template-rows` ile açılıyordu; bu özellik
  iOS Safari'nin bir kısım sürümünde güvenilir çalışmamaktadır. Panel açılımı
  her tarayıcıda çalışan bir yönteme çevrilmiş, ayrıca panel yüksekliği cevabın
  uzunluğuna göre hesaplanacak biçimde iyileştirilmiştir.
- Mobil menüde **İlçelerimiz** artık açılır/kapanır bir başlıktır. Başlığın kendisi
  ilçe dizinine gider, yanındaki ok düğmesi 14 ilçelik listeyi açıp kapatır.
  Menü kapatıldığında alt liste de sıfırlanır.

**Hız**

Yazı tipleri yalnızca sitede kullanılan karakterleri içerecek biçimde yeniden
alt kümelenmiştir: **427 KB → 115 KB (%73 azalma).** Türkçe karakterlerin
tamamı korunmuştur.
